...
def load_dataset(name: str, split_t="dense"):
    ...
    # film switched out for actor (unclear if film dataset exists separately)
    if name in [
            'cora', 'citeseer', 'pubmed', 'computers', 'photo', 'texas',
            'cornell', 'chameleon', 'actor', 'squirrel'
    ]:
        ...
        bg.num_node_features = data.num_node_features
    ...
    # TODO
        ...
        if os.path.exists(savepath):
            bg = torch.load(savepath, map_location="cpu")
            return bg
        x, y, ei, ea, mask = JacobiConv.dataset_image.load_img(name)
        mask = mask.flatten()
        bg = BaseGraph(x, ei, ea, y, mask)
        torch.save(bg, savepath)
        return bg
